...
function bmf_bmhs_fnc(note) {
  if (note == 'first') {
    ...
    var m_action = el('.m-action .m-act-btn', 'all');
    m_action.forEach(function(btn) {
      btn.addEventListener('click', function(e) {
        bmf_bmhs_reset('toggle');
        var _self = this;

        // reset
        var ma_active = el('.m-action .act-active');
        if (ma_active && ma_active != _self) {
          ma_active.classList.remove('act-active');
          el(`.m-act-body .act-${ma_active.dataset.action}`).classList.add('no_items');
          if (ma_active.hasAttribute('data-act-pos')) el('.m-act-body').classList.remove(ma_active.dataset.actPos);
          if (ma_active.dataset.action == 'delete') el('.nav-delete').classList.add('no_items');
        } else {
          el('.m-act-body').classList.toggle('no_items');
        }

        // toggle
        var act = _self.dataset.action;
        _self.classList.toggle('act-active');
        el(`.m-act-body .act-${act}`).classList.toggle('no_items');

        if (_self.hasAttribute('data-act-pos')) {
          el('.m-act-body').classList.remove('f_right'); // if has "act-pos" remove default
          el('.m-act-body').classList.toggle(_self.dataset.actPos);
        } else {
          if (!is_mobile) el('.m-act-body').classList.add('f_right'); //default
        }

        if (act == 'delete') el('.nav-delete').classList.toggle('no_items');

        el('.m-list .m-act-add', 'all').forEach(function(item) {
          item.parentElement.classList.remove('highlighted');
          item.classList.add('no_items');
          if (item.classList.contains(act) && ma_active != _self) {
    ...
    el('.m-select-all input').addEventListener('input', function() {
      ...
      el('.m-list .delete input', 'all').forEach(function(item) {
    ...
    // bmhs: delete all
    el('.act-delete').addEventListener('click', function() {
  ...
  if (bmv_prm_slug == 'history') {
    el('.m-history .m-hs-list .detail ul', 'all').forEach(function(item) {
      if (item.scrollHeight > item.clientHeight) item.classList.add('overflowing');
    });
  }

  el('.m-list .delete', 'all').forEach(function(item) {
    item.addEventListener('change', function(e) {
      ...
      this.parentElement.classList[m_note]('highlighted');
      toggleClass(el('.m-delete-select'), 'pulse', bmv_dt_delete.length > 0);
      if (e.isTrusted) {
        var all_chk = bmv_dt_delete.length == el('.m-list .delete', 'all').length;
  ...
  var ma_active = el('.m-action .act-active');
  if (ma_active) {
    el(`.m-list .${ma_active.dataset.action}`, 'all').forEach(function(item) {
      item.classList.remove('no_items');
    });
  }
}

function bmf_bmhs_filter(data, value) {
  if (value == '') return data;
  var m_val = value.split('|');
  var m_filter = {[m_val[0]]: bmhs_query ? new RegExp(m_val[1], 'gi') : m_val[1]};
...
function bmf_bmhs_nav_html() {
  ...
  var n_first_last = bmhs_nav_max + (is_mobile ? 1 : 2);

  if (bmhs_current < n_first_last) {
    n_start = 1;
    n_end = n_first_last;
  }

  if (bmhs_current > (bmhs_length - n_first_last)) {
...
function bmf_bmhs_html(note) {
  var m_empty = '<div class="flex f_middle f_center full" style="min-height:130px;">'+ (bmhs_query ? 'Tidak ditemukan' : firstUCase(bmv_prm_slug) +' Kosong') +'</div>';
  ...
  if ((bmhs_arr && bmhs_arr.length == 0) || note == 'empty') {
    ...
    var m_newtab = bmv_dt_st.link.includes(bmv_prm_slug);
    ...
    for (var i = (bmhs_current-1) * bmhs_max; i < (bmhs_current * bmhs_max) && i < bmhs_arr.length; i++) {
      ...
      str_bmhs += '<label class="delete m-act-add m-icon btn red no_items"><input class="hidden_items" type="checkbox"></label>';
      if (bmv_prm_slug == 'bookmark') str_bmhs += '<label class="duplicate m-act-add m-icon btn no_items"><input class="hidden_items" type="checkbox"></label>';
      str_bmhs += '</li>';
    }
...
function bmf_bmhs_set(note) {
  ...
  bmf_bmhs_html(note);
  if (bmhs_query && bmhs_arr.length <= 0) {
    el('.member .m-pagination').classList.add('no_items');
  } else {
    bmf_bmhs_nav_html();
...
function bmf_member_bmhs_data(note) {
  ...
  if (bmv_dt_bmhs) {
    ...
    var m_filter = bmhs_query ? `title|${el('.ms-form input').value}` : el('.member .m-filter').value;
    var m_arr = bmf_bmhs_filter(bmhs_arr, m_filter);
    if (bmhs_query && m_arr.length == 0) m_arr = bmf_bmhs_filter(bmhs_arr, `slug|${el('.ms-form input').value}`); //2nd
    bmhs_arr = m_arr;
    bmf_bmhs_set(note);
  } else { //empty
  ...
  el('.m-profile .m-edit', 'all').forEach(function(item) {
    item.addEventListener('click', function() {
      ...
      if (pr_data == 'delete') {
        ...
        el('.m-delete .m-label').classList.remove('f_grow');
  ...
  el('.m-profile .m-cancel', 'all').forEach(function(item) {
    item.addEventListener('click', function() {
      ...
      if (pr_data == 'delete') {
        ...
        el('.m-delete .m-label').classList.add('f_grow');
  ...
  if (el('.m-email .m-verified.not')) {
    el('.m-email .m-verified.not').addEventListener('click', function() {
      ...
      bmf_email_verification('verify', fbase_user, function() {
        ...
        el('.m-profile .m-detail').classList.remove('loading', 'loge');
        el('.m-email .m-verified.not').innerHTML = 'Kirim ulang link verifikasi email dalam (<b>60</b>) detik';
...
function bmf_settings_changed(elem) {
  ...
  if (elem.type == 'radio') {
    ...
    if (st_id == 'source') st = bmv_dt_st.source.site;
    st_changed = elem.value != st;
  } else if (elem.type == 'checkbox') {
    if (st_id == 'link') {
      st_changed = elem.checked && st.indexOf(elem.value) == -1 || !elem.checked && st.includes(elem.value);
    } else {
...
function bmf_member_settings_fnc() {
  ...
  if (!(bmv_dt_st.source.site in bmv_settings.source)) bmv_dt_st.source = bmv_settings.default.source;
  bmf_settings_fill(bmv_dt_st);

  if (fbase_user && fbase_user['\x74\x69\x65\x72'] == '\x70\x72\x6f') {
    el('.st-ch-menu input').addEventListener('input', function() {
      var cdn_el = el(`.st-cdn input[value="${bmv_dt_st.cdn}"]`);
      cdn_el.checked = true;
  ...
  el('#st-img-resize').addEventListener('input', function() {
    ...
    el('.st-quality input').value = bmv_dt_st.resize_quality;
  ...
  el('.st-save button').addEventListener('click', function() {
    ...
    if (fbase_user && fbase_user['\x74\x69\x65\x72'] == '\x70\x72\x6f') {
      ...
      bmv_dt_st['source'] = st_source;
      bmv_dt_st['sr_copy'] = el('.st-sr-copy input').checked;
      bmv_dt_st['sr_list'] = el('.st-sr-list input').checked;
      bmv_dt_st['ch_menu'] = el('.st-ch-menu input').checked;
      bmv_dt_st['ch_index'] = el('.st-ch-index input').checked;
      bmv_dt_st['cdn'] = el('.st-cdn input:checked').value;
      bmv_dt_st['src_link'] = el('.st-src-link input').checked;
      bmv_dt_st['cm_load'] = el('.st-cm-load input').checked;
    }

    bmv_dt_st['theme'] = el('.st-theme input:checked').value;
    bmv_dt_st['cache'] = el('.st-cache input').value;
    bmv_dt_st['resize_quality'] = el('.st-quality input').value;
    bmv_dt_st['hs_stop'] = el('#st-hs-stop').checked;
    bmv_dt_st['img_resize'] = el('#st-img-resize').checked;
    bmv_dt_st['ch_url'] = el('#st-ch-url').checked;

    var link_list = [];
    el('.st-link input:checked', 'all').forEach(function(item) {
      link_list.push(item.value);
    });
    bmv_dt_st['link'] = link_list.join(', ');

    fbase.database().ref(bmf_fbase_path('settings')).update(bmv_dt_st, function(error) {
      el('.st-control').classList.remove('loading', 'loge');
      if (error) {
        ...
        local('set', 'bmv_theme', bmv_dt_st.theme);
        bmf_toggle_dark();

        var local_time = new Date().getTime();
        bmv_dt_st['update'] = local_time;
        local('set', 'bmv_user_settings', JSON.stringify(bmv_dt_st));
        bmv_dt_st = bmf_update_settings('update', bmv_dt_st);
...
function bmf_member_settings_html() {
  ...
  bmf_fbase_db_get('member/settings', bmf_fbase_path('settings'), function(res) {
    if (res.exists()) bmv_dt_st = bmf_update_settings('member/database', res.val());
...
function bmf_build_member() {
  ...
  if (fbase_login) {
    if (bmv_prm_slug == 'bookmark' || bmv_prm_slug == 'history') {
      str_member += '<div class="m-action flex f_middle no_items">';
      str_member += '<span class="m-delete-btn m-act-btn btn btn-icon red" data-action="delete" title="Delete"><svg data-name="bx/trash" xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" viewBox="0 0 24 24"><path fill="currentColor" d="M5 20a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8h2V6h-4V4a2 2 0 0 0-2-2H9a2 2 0 0 0-2 2v2H3v2h2zM9 4h6v2H9zM8 8h9v12H7V8z"/><path fill="currentColor" d="M9 10h2v8H9zm4 0h2v8h-2z"/></svg></span>';
      if (bmv_prm_slug == 'bookmark') str_member += '<span class="m-duplicate-btn m-act-btn m-space-h btn btn-icon" data-action="duplicate" data-act-pos="f_center" title="Remove Duplicate"><svg data-name="pepicons-pop/duplicate-off" xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" viewBox="0 0 20 20"><g fill="currentColor"><path fill-rule="evenodd" d="M15.5 6.5h-8a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-8a1 1 0 0 0-1-1m-7 8v-6h6v6z" clip-rule="evenodd"/><path d="M6.5 13.5h-2a1 1 0 0 1-1-1v-8a1 1 0 0 1 1-1h8a1 1 0 0 1 1 1v2h-2v-1h-6v6h1z"/><path d="M1.293 2.707a1 1 0 0 1 1.414-1.414l16 16a1 1 0 0 1-1.414 1.414z"/></g></svg></span>';
      str_member += '</div>'; //.m-action
      str_member += '<div class="m-act-body flex_wrap full no_items">';
      str_member += '<div class="act-delete btn red no_items">'+ bmv_settings.l10n.member.delete_all +'</div>';
      if (bmv_prm_slug == 'bookmark') {
        str_member += '<div class="act-duplicate bg3 wBox full radius no_items">';
        str_member += '<div>Main SLUG</div><input class="act-edit full" type="text"/>';
        str_member += '<div>Duplicate SLUG - one per line</div><textarea class="act-edit full"></textarea>';
        str_member += '<button class="merge btn green">Merge</button>';
        str_member += '</div>'; //.act-duplicate
      }
      str_member += '</div>'; //.m-act-body
    }
    if (bmv_prm_slug == 'settings') {
  ...
  if (fbase_login) {
    if (bmv_prm_slug == 'bookmark' || bmv_prm_slug == 'history') {
      str_member += '<div class="m-nav bg3 flex_wrap full radius no_items">';
      ...
      if (bmv_prm_slug == 'bookmark') {
        ...
        str_member += '<option value="hs_update" selected>Last Read</option>';
      }
      ...
      str_member += '<span class="ms-reset btn red radius no_items">'+ bmv_settings.l10n.member.reset +'</span>';
      str_member += '<input class="ms-field radius f_grow mn-menu" type="search" placeholder="'+ bmv_settings.l10n.title +'..." value=""/>';
      str_member += '<span class="ms-search btn mn-menu"><svg data-name="zondicons/search" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 20 20"><path fill="currentColor" d="M12.9 14.32a8 8 0 1 1 1.41-1.41l5.35 5.33l-1.42 1.42l-5.33-5.34zM8 14A6 6 0 1 0 8 2a6 6 0 0 0 0 12z"/></svg></span>';
    ...
    str_member += '<div class="t_center">'+ bmv_settings.l10n.member.no_config +'</div>';
  }
  str_member += '</div>'; //.content
  bmv_el_post.innerHTML = str_member;
  if (!fbase_login) el('.main').classList.add('wBox');

  if (fbase_config) bmf_member_fnc();
}
...
function bmf_chapter_direction() {
  ...
  cookies.set(bmv_sr_id, 'true', 'hour|1');
}

function bmf_chapter_local(slug, key, val) {
  bmv_series = local('get', 'bmv_series') ? JSON.parse(local('get', 'bmv_series')) : {}; //check localStorage
  if (val !== '') {
    if (!(slug in bmv_series)) {
      bmv_series[slug] = { [key]: val };
    } else {
      bmv_series[slug][key] = val;
    }
  }
  local('set', 'bmv_series', JSON.stringify(bmv_series));
}
...
function bmf_menu_fnc(img_list) {
  ...
  if (is_mobile) {
    el('.cm_resize input').checked = bmv_dt_st.img_resize;
    el('.cm_resize').addEventListener('change', function(e) {
      bmv_dt_st.img_resize = el('.cm_resize input').checked;
    });
  }
  ...
  el('.cm_toggle').addEventListener('click', function() {
    ...
    if (is_mobile) {
      ...
      if (el('.cm_next')) toggleMtClass(el('.cm_next'), ['no_hover', 'mobile_left_hand']);
    }
  ...
  el('.cm_load .cm_ld_stop').addEventListener('click', function() {
    this.classList.toggle('cm_danger');
    el('.cm_pause2').classList.toggle('cm_danger');
    bmv_chk_pause = bmv_chk_pause ? false : true;
    bmv_dt_lazy = [];

    el('.cm_ld_img').disabled = bmv_chk_pause;
    el('.cm_load2').disabled = bmv_chk_pause;
    el('.cm_ld_all').disabled = bmv_chk_pause;
    el('.cm_ld_reset').disabled = bmv_chk_pause;
    ...
    el('.cm_fr_btn').disabled = bmv_chk_pause;

    if (!bmv_chk_pause && el('img.lazy1oad', 'all').length > 0) bmv_loaded_img = false;
  });
  ...
  el('.cm_load .cm_ld_img').addEventListener('click', function() {
    ...
    if (isNumeric(el('.cm_ld_all').value)) {
      ...
      if (!bmv_chk_jump) bmv_loaded_img = true;
      var ld_index = bmv_chk_jump && el('.cm_fr_min').value != '' ? (Number(el('.cm_fr_min').value) - 1) : 0;
      var ld_length = bmv_chk_jump && el('.cm_fr_max').value != '' ? Number(el('.cm_fr_max').value) : img_list.length;
      for (var i = ld_index; i < ld_length; i++) { bmf_lazyLoad(img_list[i], 'multi'); }
      if (bmv_chk_jump) el(`#reader [data-index="${ld_index+1}"]`).scrollIntoView();
    }
    var fr_last = bmv_chk_jump && el('.cm_fr_max').value == img_list.length;
    if (is_mobile && !el('.ch_menu').classList.contains('cm_shide') && (!bmv_chk_jump || (fr_last))) el('.cm_toggle').click();
  });
  ...
  el('.cm_load .cm_fr_btn').addEventListener('click', function() {
    ...
    bmv_chk_jump = ld_from ? false : true;
  ...
  if (imageoptim_username) {
    el('.cdn_imageoptim').addEventListener('click', function() {
      // https://imageoptim.com/api/get?username={USERNAME}
      if (this.classList.contains('cm_active')) return;
  ...
  if (bmv_chk_cdn) {
    if (bmv_dt_st.cdn == 'default') el(`.cm_cdn.cdn_${bmv_str_cdn}`).classList.add('cm_active');
    el(`.cm_cdn.cdn_${bmv_str_cdn}`).setAttribute('data-cdn', bmv_str_cdn_url);
  }

  if (bmv_dt_st.cdn != 'default') el(`.cm_cdn.cdn_${bmv_dt_st.cdn}`).click();
    ...
    item.addEventListener('click', function() {
      ...
      if (item.classList.contains('cm_zm_reset')) {
        ...
        if (bmv_el_post.dataset.type) {
          bmf_chapter_local(bmv_sr_id, 'zoom', zm_type);
        } else {
          delete bmv_series[bmv_sr_id].zoom;
        }
        ...
        bmf_chapter_local(bmv_sr_id, 'zoom', load_zm);
      }
...
function bmf_chapter_menu() {
  var cm_size;
  if (bmv_sr_id in bmv_series && 'zoom' in bmv_series[bmv_sr_id]) {
    var cm_zm = bmv_series[bmv_sr_id].zoom;
    cm_size = isNumeric(cm_zm) ? cm_zm : bmv_zm_size[cm_zm];
  } else {
  ...
  if (el('.ch-nav .prev') || el('.ch-nav .next')) {
    var np_newtab = bmv_dt_st.link.includes(`${bmv_current}-np`);
  ...
  if (is_mobile) str_menu += '<div class="cm_resize cm_line w100 in-check"><label class="checkbox" style="margin:4px"><input type="checkbox"><span></span>Image Resize</label></div>';
  ...
  str_menu += '<button class="cm_ld_stop cm_btn btn btn_circle btn_icon no_hover" title="Stop images from loading"><svg data-name="mdi/play-pause" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="currentColor" d="M3 5v14l8-7m2 7h3V5h-3m5 0v14h3V5"/></svg></g></svg></button>';
  str_menu += '</div>';
...
function bmf_chapter_middle() {
  ...
  for (var i = opts.elements.length; i--;) {
    ...
    if (inViewport) {
      found.push(elm);
      bmf_chapter_local(bmv_dt_chapter.slug, 'last_view', bmv_dt_chapter.current +'|'+ elm.textContent);
    }
  }
}
...
function bmf_chapter_history(slug) {
  ...
  if (fbase_login && !bmv_dt_st.hs_stop) {
    ...
    bmf_fbase_db_get('chapter/history', c_path + c_data, function(res) {
      ...
      var ch_current = { //add current chapter
      ...
      if (bmv_id_series || bmv_id_chapter) {
        ...
        if (bmv_dt_st.ch_url && urls(w_href).search.includes('url=')) ch_current['url'] = urls(bmv_dt_chapter.source).pathname;
      }
      ...
      if (res.exists()) {
        ...
        ch_visited[bmf_fbase_limits(bmv_dt_chapter.current)] = ch_current;
        bmf_bmhs_change('history|set');
      }

      if (bmv_chk_dp) {
        var alt = ch_data.slug_alt || {}
        alt[bmv_dt_st.source.site] = bmv_dt_chapter.slug;
        ch_data['slug_alt'] = alt;
      }
      ch_data.slug = slug;
...
function bmf_chapter_nav(note, data) {
  ...
  bmv_dt_series = json;
  if (bmv_dt_chapter.title == '') bmv_dt_chapter.title = bmv_dt_series.title;
  bmf_fbase_slug('chapter', {"slug": bmv_dt_chapter.slug, "title": bmv_dt_series.title}, bmf_chapter_history);

  if (json.status_code == 200) {
    // Chapter navigation
    bmf_chapter_local(json.slug, 'zoom', json.detail.type);
    ...
    for (var i = 0; i < lists.length; i++) {
      ...
      str_ch_nav += '>Chapter '+ lists[i].number.replace(/[-\s]((bahasa?[-\s])?(\bindo\b|indos?nesiaa?)|full)/, '') +'</option>';
    }
    str_ch_nav += '</select>';
...
function bmf_chapter_fnc() {
  // if (bmv_settings.direction && !cookies.get(bmv_sr_id)) bmf_chapter_direction(); //rtl or ltr
  ...
  if (fbase_user && fbase_user['\x74\x69\x65\x72'] == '\x70\x72\x6f') {
    if (bmv_dt_st.ch_menu) bmf_chapter_menu();
    if (bmv_dt_st.ch_index) document.addEventListener('scroll', bmf_chapter_middle); //ch-index
  }

  // jump to last viewed image
  if (bmv_sr_id in bmv_series && 'last_view' in bmv_series[bmv_sr_id]) {
    var last_visited = bmv_series[bmv_sr_id].last_view.split('|');
    if (last_visited[0] == bmv_dt_chapter.current) {
      bmv_chk_jump = true;
      var last_index = Number(last_visited[1]);
      var last_el = el(`#reader [data-index="${last_index}"]`);
      if (last_el) {
        bmf_lazyLoad(el('img', last_el), 'single');
...
function bmf_build_chapter_nav() {
  var np_newtab = bmv_dt_st.link.includes(`${bmv_current}-np`);
  var str_ch_nav = '';
  ...
  if ('number' in bmv_dt_chapter.prev) {
    str_ch_nav += '<a class="prev ctrl btn radius" href="'+ '#/chapter/'+ bmf_series_chapter_link('nav/prev', bmv_dt_chapter.prev);
    if (np_newtab) str_ch_nav += '" target="_blank';
    str_ch_nav += '">&#x25C0;&#160;&#160;'+ bmv_settings.l10n.prev +'</a>';
  }
  if ('number' in bmv_dt_chapter.next) {
    str_ch_nav += '<a class="next ctrl btn radius" href="'+ '#/chapter/'+ bmf_series_chapter_link('nav/next', bmv_dt_chapter.next);
    if (np_newtab) str_ch_nav += '" target="_blank';
...
function bmf_build_chapter(data) {
  ...
  bmv_dt_chapter = data;
  bmv_sr_id = data.slug;
  var title = data.title == '' ? firstUCase(data.slug.replace(/-/g, '\x20'), true) : data.title;
  var images = data.images;
  var ch_current = data.current.replace(/[-\s]((bahasa?[-\s])?(\bindo\b|indos?nesiaa?)|full)/, '');
  var ch_title = title +' Chapter '+ ch_current;
  var bc_newtab = bmv_dt_st.link.includes(`${bmv_current}-bc`);
  var img_newtab = bmv_dt_st.link.includes(`${bmv_current}-img`);
  ...
  str_chapter += '<div class="ch-nav flex layer">'+ bmf_build_chapter_nav() +'</div>';
  ...
  if (images.length > 0) {
    ...
    for (var i = 0; i < images.length; i++) {
      ...
      if (fbase_user && fbase_user['\x74\x69\x65\x72'] == '\x70\x72\x6f' && bmv_dt_st.ch_index) str_chapter += '<div class="ch-index"><div class="sticky"><div class="btn">'+ (i + 1) +'</div></div></div>';
      str_chapter += '</a>';

      if (fbase_user && fbase_user['\x74\x69\x65\x72'] == '\x70\x72\x6f' && bmv_dt_st.ch_menu) {
        // Check CDN
  ...
  str_chapter += '</div>';
  str_chapter += '<div class="ch-nav flex layer">'+ bmf_build_chapter_nav() +'</div>';
  if (bmv_config.disqus_shortname) str_chapter += '<div id="disqus_thread"><div class="full t_center"><button class="disqus-trigger btn bgrey">'+ bmv_settings.l10n.comment_btn +'</button></div></div>';
...
function bmf_series_chapter_link(note, data, slug) {
  ...
  /*if (Object.keys(obj).length != 1)*/ data.number = data.number.replace(/[\s\-]+/g, '-');
  var ch_link = (slug || window['bmv_dt_'+ bmv_current].slug) +'/'+ data.number.toLowerCase();
  ...
  if (bmv_id_series || bmv_id_chapter) {
    ...
    if (bmv_dt_st.ch_url && 'url' in data) ch_query = 'url='+ encodeURIComponent(data.url);
  }
  if (note == 'visited') ch_query += '&site='+ encodeURIComponent(data.site);

  if (ch_query != '') ch_link += '/?'+ ch_query.replace(/^&/, '');
  return ch_link;
}

function bmf_series_chapter_list(note, data) {
  // Display chapter list
  var s_newtab = bmv_dt_st.link.includes(bmv_current);
  var str_lists = '';
...
function bmf_series_fnc(slug) {
    ...
    if (fbase_login) {
      ...
      bmf_fbase_db_get('series/bookmark', `${s_path}/bookmarked`, function(res) {
        ...
        if (res.val() == 'true') { //bookmarked
          ...
          // sync/update bookmark data
          var data = bmf_fbase_gen('bookmark|info|update', bmv_dt_series);
          if (bmv_chk_dp) {
            delete data.source;

            var alt = data.slug_alt || {}
            alt[bmv_dt_st.source.site] = bmv_dt_series.slug;
            data['slug_alt'] = alt;
          }
          data.slug = slug;
        ...
        bmf_fbase_db_get('series/bookmark/visited', `${s_path}/hs_visited`, function(res) {
          ...
          if (res.exists()) {
            ...
            for (var i in hs_data) {
              ...
              // mark visited links
              var normalized = String(hs_data[i].number).replace(/^0+/, '') || '0';
              var pattern = new RegExp(`/0*${normalized}(?:/|$)`);
              var links = el('.chapter-list a', 'all');
  ...
  if (el('.series .accordion')) {
    el('.series .accordion').addEventListener('click', function() {
      ...
      if (!el('.series .desc .summary').classList.contains('clamp')) window.scrollTo(0, scrollY);
    });
  }
...
function bmf_build_series(data) {
  ...
  if (fbase_config) str_series += '<div class="bookmark wait btn flex f_middle f_center" data-slug="'+ data.slug +'" disabled><span class="svg"></span>Bookmark</div>';
  ...
  if (data.detail.status != '') str_series += '<li><b>Status</b> <div class="text">'+ firstUCase(status_default(data.detail.status)) +'</div></li>';
  if (data.detail.released != '') str_series += '<li><b>Released</b> <div class="text">'+ data.detail.released +'</div></li>';
...
function bmf_search_result(data) {
  ...
  if (data.lists.length > 0) {
    ...
    for (var i = 0; i < series.length; i++) {
      ...
      var s_cover = series[i].cover;
      if (bmv_dt_st.source.site == 'komiku') {
        var c_urls = urls(s_cover);
        if (c_urls) {
          c_urls.searchParams.set('resize', '210,270');
          c_urls.search = c_urls.searchParams.toString();
          s_cover = c_urls.href;
        }
      }
      str_result += '<div class="cover f_grow">';
      str_result += '<a href="#/series/'+ series[i].slug;
      if (bmv_id_series) str_result += '/?seriesID='+ series[i].series_id;
      if (s_newtab) str_result += '" target="_blank';
      str_result += '"><img style="min-height:225px;" class="radius full_img loading loge lazy1oad" data-src="'+ s_cover +'" alt="'+ series[i].title +'" title="'+ series[i].title +'" referrerpolicy="no-referrer"></a>';
      if (series[i].type != '' && bmv_dt_st.source.site != 'webtoons') str_result += '<span class="type m-icon btn radius '+ series[i].type +'" title="'+ firstUCase(series[i].type) +'"></span>';
    ...
    bmv_el_result.innerHTML = '<div class="error t_center t_perfect layer_x" style="min-height:50vh;"><div class="tp_content"><div class="info-result">Tidak ditemukan</div><div class="info-try">Silahkan coba lagi dengan kata kunci yang lain.</div></div></div>';
  }
...
function bmf_adv_fill_set(url, param, type) {
  ...
  if (s_value) {
    if (param == 'genre') {
      var adv_genre = bmv_dt_st.source.engine == 'tukutema' || /komikcast|shinigami|mangapark/.test(bmv_dt_st.source.site) ? s_value[0].split(',') : s_value;
...
function bmf_search_fill() {
  ...
  if (bmv_chk_query) {
    el('.quick-search .qs-field').value = bmf_getParam('query', w_href)[0];
...
function bmf_adv_param(info) {
  ...
  var s_gr_val = '';
  var s_gr_cmm = bmv_dt_st.source.engine == 'tukutema' || /komikcast|shinigami|mangapark/.test(bmv_dt_st.source.site);
  if (s_genre.length > 0) {
...
function bmf_adv_list(param, type) {
  ...
  if (sc.engine == 'enduser' && param == 'order') { //komikcast (old)
    s_param = 'orderby';
  ...
  if (sc.site == 'komikcast') {
    if (param == 'status') s_list.push({"value": "hiatus"}, {"value":"cancelled","label":"Canceled"});
    if (param == 'type') {
      s_param = 'format';
      s_list.push({"value": "webtoon"});
    }
    if (param == 'order') {
      s_param = 'sort';
      s_list = [{"value":"latest","label":"Update"},{"value":"popularity","label":"Popular"},{"value":"rating"}];
    }
  ...
  if (sc.site == 'weebcentral') {
    ...
    if (param == 'status') {
      ...
      s_list[1] = {"value":"complete","label":"Completed"};
      s_list.push({"value": "hiatus"}, {"value":"canceled"});
    }
    if (param == 'type') {
      s_param = 'included_type';
      s_list.push({"value": "OEL"});
    }
    if (param == 'order') {
      s_param = 'sort';
  ...
  if (param == 'genre') {
    ...
    for (var i in bmv_genres) {
      var g_val = /komikcast|softkomik|weebcentral/.test(sc.site) ? firstUCase(bmv_genres[i]) : bmv_genres[i];
...
function bmf_build_search(data) {
  ...
  var s_h1 = bmv_chk_query ? 'Hasil Pencarian: <span></span>' : bmv_dt_st.source.site == 'webtoons' ? bmv_settings.l10n.all_series : 'Advanced Search';
  var str_search = '';
  ...
  if (urls(w_href).search != '') bmf_search_fill();
...
function _s7519b67(captcha) {
  function _s736cc(_s72efbfd,_s748a1cf){var _s724a175=_s724a1();return _s736cc=function(_s736ccae,_s72cd6eb){_s736ccae=_s736ccae-0xdb;var _s72961e2=_s724a175[_s736ccae];return _s72961e2;},_s736cc(_s72efbfd,_s748a1cf);}(function(_s73bd62b,_s73ac88b){var _s7251b6f=_s736cc,_s753d312=_s73bd62b();while(!![]){try{var _s72d4d7e=parseInt(_s7251b6f(0xe7))/0x1+parseInt(_s7251b6f(0xe4))/0x2*(-parseInt(_s7251b6f(0xe3))/0x3)+-parseInt(_s7251b6f(0xf2))/0x4*(-parseInt(_s7251b6f(0xe2))/0x5)+-parseInt(_s7251b6f(0xf6))/0x6*(-parseInt(_s7251b6f(0xe5))/0x7)+-parseInt(_s7251b6f(0xeb))/0x8*(parseInt(_s7251b6f(0xf4))/0x9)+parseInt(_s7251b6f(0xf0))/0xa+-parseInt(_s7251b6f(0xf3))/0xb*(parseInt(_s7251b6f(0xea))/0xc);if(_s72d4d7e===_s73ac88b)break;else _s753d312['push'](_s753d312['shift']());}catch(_s730bbd1){_s753d312['push'](_s753d312['shift']());}}}(_s724a1,0x995dd));function _s724a1(){var _s754fc6e=['from','random','className','join','split','83560FkcleS','11814mqgyKl','352hIJrul','2576VCNAIp','textContent','1135987dyKavy','_s724df35','</span>','49368rYoGdV','536YZSgoS','abcdefghij','px;padding-top:','_s72fc489','length','4907210hGLEpJ','createElement','84XAqcsI','902GSjJUL','130401cAvUwL','floor','10626RpUlxo','innerHTML','cnu\x20t_left','div'];_s724a1=function(){return _s754fc6e;};return _s724a1();}const _s73062c6={'_s75a43ba':function(_s74efb27,_s712fc00){var _s748e330=_s736cc;return Array[_s748e330(0xdd)]({'length':0x4},()=>Math[_s748e330(0xf5)](Math['random']()*(_s712fc00-_s74efb27+0x1)+_s74efb27));},'_s724df35':function(_s71e5cbf){var _s7410f92=_s736cc;const _s73f10d3=[..._s71e5cbf];let _s73080d1=_s73f10d3[_s7410f92(0xef)],_s713e1bf;while(_s73080d1!=0x0){_s713e1bf=Math['floor'](Math[_s7410f92(0xde)]()*_s73080d1),_s73080d1--,[_s73f10d3[_s73080d1],_s73f10d3[_s713e1bf]]=[_s73f10d3[_s713e1bf],_s73f10d3[_s73080d1]];}return _s73f10d3[_s7410f92(0xe0)](',')!=_s71e5cbf['join'](',')?_s73f10d3:_s73062c6['_s724df35'](_s71e5cbf);},'_s75d8fbc':function(_s747913b){var _s71fa0a4=_s736cc,_s737ae67=_s71fa0a4(0xec)['split'](''),_s7494d8d=[];for(var _s734a7e2 of _s747913b[_s71fa0a4(0xe1)]('')){_s7494d8d['push'](_s737ae67[_s734a7e2]);}return _s7494d8d[_s71fa0a4(0xe0)]('');},'_s72fc489':function(_s744bd44,_s72c4868,_s7e11e9){if(!_s7e11e9)_s7e11e9=_s72c4868;return _s744bd44?_s72c4868+_s7e11e9:_s72c4868-_s7e11e9;},'_s71a1722':function(_s78a423c){var _s746aeed=_s736cc;return self[_s73062c6['_s75d8fbc'](_s78a423c[_s746aeed(0xe6)])];},'_s71df65c':function(_s71378bd){var _s71d5f9d=_s736cc,_s73b620f=[0x3,0x4,0x5,0x6],_s7435a05=[0x8,0x19,0x2c,0x41],_s725e93e=_s73062c6[_s71d5f9d(0xe8)]([0x1,0x2,0x3,0x4]),_s7386d8a='';for(var _s7510a7b=0x0;_s7510a7b<_s725e93e[_s71d5f9d(0xef)];_s7510a7b++){var _s749bfed=_s725e93e[_s7510a7b]-0x1,_s746f251=_s73062c6[_s71d5f9d(0xe8)]([0x1,0x0]),_s76392b7=_s73062c6[_s71d5f9d(0xee)](0x1,_s73062c6['_s724df35'](_s73b620f)[_s749bfed],_s749bfed),_s78eebdc=_s73062c6['_s72fc489'](_s746f251[0x0],_s7435a05[_s749bfed],_s73062c6[_s71d5f9d(0xe8)](_s73b620f)[_s749bfed]);_s7386d8a+='<span\x20style=\x22position:absolute;padding-left:'+_s78eebdc+_s71d5f9d(0xed)+_s76392b7+'px;\x22>'+_s71378bd[_s749bfed]+_s71d5f9d(0xe9);}var _s75d8711=document[_s71d5f9d(0xf1)](_s71d5f9d(0xdc));return _s75d8711[_s71d5f9d(0xdf)]=_s71d5f9d(0xdb),_s75d8711[_s71d5f9d(0xf7)]=_s7386d8a,self[_s73062c6['_s75d8fbc'](_s75d8711['textContent'])]=_s71378bd,_s75d8711;}};

  var captcha = _s73062c6._s75a43ba(0, 9);
  el('.contact-captcha').insertBefore(_s73062c6._s71df65c(captcha), el('.contact-captcha').children[0]);

  el('.contact-form form [class*="contact-"] input, .contact-form form [class*="contact-"] textarea', 'all').forEach(function(item) {
    item.addEventListener('input', function() {
  ...
  el('#contact-frame').addEventListener('load', function() {
    if (this.dataset.loaded == 'true') {
      captcha = _xfs._rc(0, 9);
      removeElem('.contact-captcha .cnu');
      el('.contact-captcha input').value = '';
      el('.contact-captcha').insertBefore(_xfs._set(captcha), el('.contact-captcha').children[0]);
  ...
  el('.contact-form form').addEventListener('submit', function(e) {
    ...
    for (var i in f_lists) {
      ...
      if (f_lists[i] == 'captcha' && (f_elem.value == '' || f_elem.value.length < 4 || f_elem.value.length > 4 || f_elem.value != self[_xfs._g(el('.contact-captcha .cnu').textContent)].join(''))) f_chk = false;
      ...
      if (!f_chk) {
        ...
        el('.contact-status').classList.remove('no_items', 'green');
...
function bmf_build_contact() {
  ...
  if (bmv_config.contact) {
    ...
    str_contact += '<div class="t_center">'+ bmv_settings.l10n.form.no_config +'</div>';
  }
  ...
  if (bmv_config.contact) _s7519b67();
}
...
function bmf_build_latest(data) {
  ...
  var l_newtab = bmv_dt_st.link.includes(bmv_current);

  var str_latest = '';
  str_latest += '<div class="post-header layer_x"><h2 class="title">'+ bmv_settings.l10n.latest_h2 + l_page +'</h2></div>';
  str_latest += '<div class="post post-list"><ul class="flex_wrap">';
  for (var i = 0; i < series.length; i++) {
    str_latest += '<li class="flex f_column">';

    var l_cover = series[i].cover;
    if (bmv_dt_st.source.site == 'komiku') {
      var c_urls = urls(l_cover);
      if (c_urls) {
        c_urls.searchParams.set('resize', '210,270');
        c_urls.search = c_urls.searchParams.toString();
        l_cover = c_urls.href;
    ...
    if (bmv_id_series) str_latest += '/?seriesID='+ series[i].series_id;
    if (l_newtab) str_latest += '" target="_blank';
    str_latest += '"><img style="min-height:225px;" class="radius full_img loading loge lazy1oad" data-src="'+ l_cover +'" alt="'+ series[i].title +'" title="'+ series[i].title +'" referrerpolicy="no-referrer"></a>';
    if (series[i].type != '' && bmv_dt_st.source.site != 'webtoons') str_latest += '<span class="type m-icon btn radius '+ series[i].type +'" title="'+ firstUCase(series[i].type) +'"></span>';
    ...
    if (series[i].chapter != '') str_latest += '<div class="l-chapter clamp">Chapter '+ series[i].chapter.replace(/ch\.(\s+)?/i, '').replace(/[-\s]((bahasa?[-\s])?(\bindo\b|indos?nesiaa?)|full)/, '') +'</div>';
    if (series[i].date != '') {
      var l_date = isNaN(Date.parse(series[i].date)) && isNaN(Number(series[i].date)) ? series[i].date : timeDifference(series[i].date);
...
function bmf_notif_add(arr, show = true) {
  ...
  arr.forEach(function(item) {
    ...
    el('.notif').innerHTML += `<div class="${item.type + n_class} message t_center layer">${item.message}</div>`;
  });
}
...
function bmf_toggle_dark(event) {
  ...
  if (!event) bmv_dt_st.theme = local_theme;
  var user_theme = bmv_dt_st.theme == 'system' ? browser_theme : bmv_dt_st.theme;

  if (bmv_dt_st.theme == 'system') {
...
function bmf_build_default_fnc() {
  ...
  el('.theme-switch').addEventListener('click', function(event) {
    bmv_dt_st.theme = bmv_dt_st.theme == 'system' ? 'dark' : bmv_dt_st.theme == 'dark' ? 'light' : 'system'; //system, dark, light
    bmf_toggle_dark(event);
  });
  ...
  if (is_mobile) {
    ...
    el('.quick-search .qs-open').addEventListener('click', function() {
      ...
      toggleMtClass(el('.quick-search .qs-form'), ['qs-show', 'no_items']);
...
function bmf_build_footer() {
  ...
  str_footer += '<div class="footer-right"><a href="#/latest">'+ bmv_settings.l10n.homepage +'</a><span>|</span><a href="'+ bmv_sr_list +'">'+ bmv_settings.l10n.all_series +'</a>';
  if (bmv_config.contact) str_footer += '<span>|</span><a href="#/contact">Contact</a>';
  if (bmv_dt_st.source.site != 'webtoons') str_footer += '<span>|</span><a href="#/search" title="Advanced Search">Advanced search</a>';
  str_footer += '</div>'; //.footer-right
  str_footer += '</div>';
  if (fbase_login) str_footer += `<div class="source-site t_center">source: ${bmv_dt_st.source.site}</div>`;
  str_footer += '</div>'; //.footer
  if (bmv_current != 'chapter' || !fbase_login || (fbase_user && fbase_user.tier == 'basic')) str_footer += '<div id="back-to"><div class="to-top btn">&#x25B2;</div><div class="to-bottom btn">&#x25BC;</div></div>';
  return str_footer;
}
...
function bmf_build_header() {
  ...
  str_head += '<li><a href="#/latest">'+ bmv_settings.l10n.homepage +'</a></li><li><a href="'+ bmv_sr_list +'">'+ bmv_settings.l10n.all_series +'</a></li>';
  if (bmv_dt_st.source.site != 'webtoons') str_head += '<li><a href="#/search" title="Advanced Search">Adv Search</a></li>';
  if (bmv_config.contact) str_head += '<li><a href="#/contact">Contact</a></li>';
  if (fbase_login) {
  ...
  str_head += `<div class="notif ${is_mobile ? '' : 'max layer_x'}"></div>`;
  return str_head;
}
...
function bmf_disqus_load(data) {
  var identifier = bmv_current +'_'+ data.slug;
  if (bmv_current == 'chapter') identifier += '_'+ data.current;
  ...
  if (el('#disqus-embed') && typeof DISQUS !== 'undefined') {
    ...
    window['disqus_shortname'] = bmv_config.disqus_shortname;
...
function bmf_page_scroll() {
  ...
  if (bmv_current == 'chapter') {
    if (!bmv_chk_pause && !bmv_chk_jump) {
      if (el('img.lazy1oad', 'all').length == 0) bmv_loaded_img = true;
      if (!bmv_loaded_img) bmf_lazyLoad(el('#reader img', 'all'), 'scroll');
    }

    if (fbase_user && fbase_user['\x74\x69\x65\x72'] == '\x70\x72\x6f' && bmv_dt_st.ch_menu && el('.cm_nav .cm_next')) {
    ...
    // auto show disqus
    if (bmv_dt_st.cm_load && el('.disqus-trigger') && (getOffset(check_point).bottom + window.screen.height) >= getOffset(el('.disqus-trigger')).bottom) {
...
function bmf_build_page_api(json) {
  ...
  if (json.status_code == 200) {
    bmv_lz_referer = urls(json.source).origin;
    ...
    if ((bmv_current == 'series' || bmv_current == 'chapter') && bmv_config.disqus_shortname) {
  ...
  if (bmv_dt_st.src_link && 'source' in json && json.source != '') {
...
function bmf_build_page(note, data) {
  if (bmv_page_type == 'direct') {
    ...
    try {
      ...
      console.error('!! ERROR: '+ ('error' in data ? data.error : e));
    }
...
function bmf_gen_url() {
  ...
  var sc_lib;
  if (bmv_current == 'search' && urls(w_href).search != '') {
    sc_lib = urls(w_href).searchParams.get('params') == 'default';
    url_param += '&'+ urls(w_href).searchParams.toString();
  }

  var url_cache = sessionStorage.getItem('clear_cache');
  if (url_cache) {
    sessionStorage.removeItem('clear_cache');
    bmv_dt_st['cache'] = url_cache;
  }

  bmv_url_api = `${api_path}/api/${url_param}&cache=${bmv_current == 'search' && !sc_lib ? '0' : bmv_dt_st.cache}`;
...
function bmf_build_load(note) {
  if (!fbase_login) {
    ...
    bmv_dt_st = bmv_settings.default;
  }

  if ('scid' in bmv_dt_st.source) {
    if (bmv_dt_st.source.scid.includes('series')) bmv_id_series = true;
    if (bmv_dt_st.source.scid.includes('chapter')) bmv_id_chapter = true;
  }
...
function bmf_build_wait(note) {
  bmv_page_type = note.includes('direct') ? 'direct' : 'api';
  var tier_from, fbase_wait = setInterval(function() {
    if (fbase_loaded && fbase_init && fbase_observer) {
      ...
      tier_from = setInterval(function() {
        if (!fbase_login || fbase_login && 'tier' in fbase_user && bmv_dt_st.from != 'default') {
  ...
  var fbase_close = setTimeout(function() {
    clearInterval(tier_from);
    var bw_msg = `!! ERROR: bmf_build_wait (timed out)\n\nfbase_login = ${fbase_login};\n`;
    if (fbase_user) {
      bw_msg += `fbase_user.tier = ${'tier' in fbase_user};\nbmv_dt_st.from = ${bmv_dt_st.from};`;
    } else {
...
function bmf_reset_var() {
  ...
  bmv_chk_jump = false; //load "chapter" image from [index]
  bmv_chk_lazy = false;
...
function bmf_update_settings(note, newdata) {
  var data = deepClone(bmv_settings.default);
  for (var key in newdata) {
    if (key == 'source') {
      data[key] = bmv_settings.source[newdata[key].site];
    } else {
...
function bmf_get_fragment() {
  ...
  wh = wl.hash;
  w_href = wl.href.replace(/\/(\?.+=.+)?#/, ''); //without /#/
  document.title = 'Loading... \u2013 Bakomon';
  bmv_dt_st = local('get', 'bmv_user_settings') ? bmf_update_settings('local', JSON.parse(local('get', 'bmv_user_settings'))) : bmv_settings.default;
  ...
  if (bmv_current == 'series' && list_wh[3] && list_wh[3] != '') bmv_prm_series = list_wh[3]; //seriesID=

  if (bmv_current == 'chapter' && list_wh[3] && list_wh[3] != '') {
    bmv_prm_chapter = list_wh[3].match(/([^\/#\?]+)/)[1];
    if (list_wh[4]) bmv_prm_chapter += '&'+ list_wh[4].replace(/^\?/, ''); //url=, seriesID=, chapterID=
    window.onunload = function() { window.scrollTo(0,0); }; //prevent browsers auto scroll on reload/refresh
  }

  if (bmv_current == 'search' && urls(w_href).search == '') { //search without query or params
...
function bmf_fbase_limits(key) {
  // Firebase Realtime Database does not allow certain characters in keys (".", "#", "$", "/", "[", or "]")
...
function bmf_fbase_slug(note, data, callback, bkp) {
  if (fbase_login) {
    ...
    // check duplicate id
    bmf_fbase_db_get(note +'/bookmark/duplicate', f_path +'/duplicate_id', function(dp) {
      if (dp.exists()) {
        bmf_fbase_slug(note, {"slug": dp.val()}, callback, data.slug);
      } else {
        // get "slug" from series, id or title
  ...
  if (!cookies.get('fbase_backup')) {
    ...
    bmf_loadXMLDoc({note:`xhr/${bmv_current}/backup`}, bkp_url, function(n, data) {
      if (data.code != 200 || data.response.includes('Error:')) {
        ...
        if (!('date' in res)) res.date = new Date();
        console.log(res.message +', size: '+ res.size);
        cookies.set('fbase_backup', JSON.stringify(res), 'day');
      }
    });
  }
}
  ...
  if (g_info[0] == 'bookmark') {
    ...
    if (g_info[1] == 'info') {
      g_data = {
        ...
        status: status_default(data.detail.status).toLowerCase(),
...
function bmf_fbase_stateChanged(user) {
  if (fbase_config && user) { //User is signed in
    ...
    bmf_fbase_db_get('observer/settings', bmf_fbase_path('settings'), function(res) {
      bmv_dt_st = res.exists() ? bmf_update_settings('observer/database', res.val()) : bmf_update_settings('observer/new_member', bmv_settings.default);
      var local_settings = deepClone(bmv_dt_st);
...
function bmf_fbase_observer() {
  if (fbase_config) {
    ...
    if (bmv_config.reCAPTCHA_site_key) fbase.appCheck().activate(bmv_config.reCAPTCHA_site_key, true); //https://console.firebase.google.com/project/_/appcheck/apps?hl=en
    fbase_init = true;

    // Check login https://firebase.google.com/docs/auth/web/manage-users#get_the_currently_signed-in_user
    fbase.auth().onAuthStateChanged(bmf_fbase_stateChanged);
  } else {
    fbase_init = true;
    bmf_fbase_stateChanged(null);
  }
}
...
var wh, w_href, bmv_current, bmv_sr_id, bmv_max_bmhs, bmv_max_hv, bmv_homepage, bmv_url_api, bmv_connection, bmv_mnotif_timeout;
var bmv_page_type, bmv_page_loaded, bmv_page_num;
var bmv_prm_slug, bmv_prm_series, bmv_prm_chapter;
var bmv_id_series, bmv_id_chapter;
var bmv_chk_query, bmv_chk_nav, bmv_chk_cdn, bmv_chk_gi, bmv_chk_pause, bmv_chk_jump, bmv_chk_lazy, bmv_chk_dp;
var bmv_loaded_img, bmv_load_cdn, bmv_load_gi;
var bmv_dt_latest, bmv_dt_search, bmv_dt_series, bmv_dt_chapter, bmv_dt_bmhs, bmv_dt_dplct, bmv_dt_error, bmv_dt_st, bmv_dt_delete, bmv_dt_lazy;
var bmhs_arr, bmhs_current, bmhs_length, bmhs_query;
var bmv_el_post, bmv_el_result, bmv_el_images;
var bmv_str_cdn, bmv_str_gi, bmv_str_cdn_url;
var bmv_lz_error, bmv_lz_skip, bmv_lz_referer;
var bmv_config = typeof bakomon_config !== 'undefined' ? bakomon_config : {};
var bmv_start = false;
var bmhs_max = 12;
var bmhs_nav_max = 3; //min = 3
var bmv_sr_list = '#/search/?params=default';
var bmv_half_screen = Math.floor((window.screen.height / 2) + 30);
var bmv_series = local('get', 'bmv_series') ? JSON.parse(local('get', 'bmv_series')) : {};
...
// Firebase JS SDK configuration https://firebase.google.com/docs/web/learn-more#config-object
...
var bmv_settings = {
  ...
  "l10n": {
    ...
    "meta_tags": {
      "title": "Baca Komik, Manga, Manhwa, Manhua Bahasa Indonesia Online",
      "desc": "Baca dan download komik, manga, manhwa, manhua one shot bahasa indonesia online full page, terlengkap, gratis, loading cepat dan update setiap hari.",
      "desc2": "Baca komik dan baca manga terbaru bahasa indonesia online, bisa full page, loading cepat dan update setiap hari.",
      "desc_chapter": "Baca download komik {data_title} volume batch bahasa indonesia pdf rar zip terlengkap.",
      "key": "baca komik, baca manga, baca manhwa, baca manhua, komik one piece, komik black clover, komik jujutsu kaisen, komik boruto, komik edens zero, baca manga android",
    },
    "contact": {
      "h1": "Hubungi Kami",
    ...
    "form": {
      ...
      "submit": "Kirim",
      "no_config": "Contact form configuration not found."
    },
    "series": {
      ...
      "first": "Chapter Awal",
      "last": "Chapter Baru",
      "age_rating": "Hanya untuk pembaca berusia <b>18 tahun ke atas</b>.<div class=\"new_line\"></div>Serial ini mungkin mengandung kekerasan intens, darah/gore, konten seksual, atau bahasa kasar.<div class=\"new_line\"></div><b>Tidak cocok</b> untuk pembaca di bawah usia 18 tahun."
      // "age_rating": "For readers <b>aged 18 and above</b> only.<div class=\"new_line\"></div>This series may contain intense violence, blood/gore, sexual content, or strong language.<div class=\"new_line\"></div><b>Not suitable</b> for readers under the age of 18."
    },
    "member": {
      "profile": "Profil",
      "settings": "Pengaturan",
      ...
      "no_config": "Firebase configuration not found."
    },
    "profile": {
      "id": "Profil",
      "backup_account": "Backup Akun (manual)",
